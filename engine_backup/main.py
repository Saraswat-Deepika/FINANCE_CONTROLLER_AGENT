...
try:
    ...
    get_exception_reasons_for_patterns = None
...
def reconcile():
    try:
        ...
        # Step 4: AI Reasoning integration (Context-aware unmatched processing)
        if get_exception_reasons_for_patterns:
            patterns_to_send = {}
            ...
            reasons_dict = {}
            if patterns_to_send:
                reasons_dict = get_exception_reasons_for_patterns(patterns_to_send)
...
def evaluate(payload: Dict[str, Any]):
    try:
        ...
        # Parse Needs Review (Treated as Fuzzy Matched for evaluation)
        ...
        categories = ["Fully Matched", "Fuzzy Matched", "Unmatched"]
...
def ask(payload: Dict[str, Any]):
    try:
        ...
        prompt = f"""Tum ek financial analyst assistant ho. Yaha reconciliation summary aur data hai:
